...
RBI processes these batches at scheduled intervals throughout the day.

The recipient's bank receives the instruction in the next batch cycle and credits the amount.
...
NEFT was launched in 2005 and operates on a batch-based settlement system (hourly cycles). Transfers typically take 30 minutes to a few hours. There is no minimum or maximum amount, and the service has been available 24/7 since December 2019.
...
When you initiate an RTGS transfer, the instruction goes directly to RBI's central system. The system debits your bank and credits the destination bank immediately, transaction by transaction.

There is no waiting for a batch. The settlement happens in real time at the gross level.
...
RTGS was introduced in 2004 to provide real-time gross settlement for individual transactions, settling immediately (within minutes). It carries a minimum transfer amount of ₹2,00,000, has no upper limit, and has been available 24/7 since December 2020.
...
When you initiate a transfer, the instruction goes through NPCI's central switch. NPCI routes it to the destination bank, which credits the amount immediately.

The settlement happens in real time, similar to RTGS, but without the high minimum amount.

IMPS was launched by NPCI in 2010 and offers real-time settlement in a matter of seconds. It supports transfers starting from as low as ₹1 up to around ₹5,00,000 depending on the bank, and is available 24/7 throughout the year.
...
This single idea is the VPA that is arguably the most important design decision in UPI's architecture.
...
Your UPI PIN is encrypted on-device using your bank's public key before it goes anywhere. This is important. The PSP (PhonePe) never sees your actual PIN. Only your bank can decrypt it.
...
If all checks pass, your bank sends an authentication success response back to NPCI.

If anything fails — wrong PIN, insufficient balance, suspicious activity — your bank sends a failure response and the transaction stops here.
...
NPCI consolidates the confirmations and sends a final success response to PhonePe.

PhonePe shows you "Payment Successful."

Your friend's phone buzzes with a credit notification.
...
- 0.6s — On success, NPCI issues a debit instruction to the payer bank and a credit instruction to the payee bank (these can be processed in parallel).
- 0.8s — Both banks update their ledgers; confirmations are sent back to NPCI.
- 1.0s — NPCI aggregates confirmations and returns a final success to the PSP, which displays "Payment Successful." (Network conditions, load, and bank processing can extend this up to a few seconds.)
...
- RTGS required ₹2 lakh minimum.
- UPI required none of that.
- A VPA. A PIN. Done.
- The abstraction layer UPI introduced, VPAs sitting on top of account numbers, PSPs sitting on top of banks, all connected through NPCI and made the complexity invisible.
...
NEFT gave India digital transfers. IMPS gave India instant transfers. UPI gave India invisible transfers.
